...
**当前状态：与 iOS 功能对等的健康聊天客户端。** 对话、记忆/用药、搜索、召回、OCR、check-in、
首屏建议、SpokenBrief / App Shortcuts 等已落地；**Health Connect 暂缓**（大陆机普遍没有），
见下文。有意保留的其它平台差异见 `README.md`。

## Health Connect 暂缓

大陆发行版手机通常没有 Google Health Connect，也装不了依赖 Play 商店的独立 HC APK。
在没有稳定的本机健康数据源之前：

- `Features.HEALTH_CONNECT = false`——不挂健康工具、不请求 HC 权限、不引导安装。
- `HealthStore` / `HealthTools` / `HealthSituation` 代码保留，方便以后接回或换数据源。
- 重新打开时把开关改回 `true`，并确认 Manifest 权限与空态三种原因（没装 / 没授权 / 没写入方）仍分得清。

`minSdk` 仍按 28 留着（HC 官方可用下限）；开关关掉后不依赖本机是否有 HC。
...
原定按 Health Connect 官方可用下限（Android 9）。**HC 本身暂缓**（见上文「Health Connect
暂缓」），minSdk 仍留 28，避免以后接回来再改一轮兼容分支；不要求本机已装 HC。

若长期不做 HC、也不接其它健康 SDK，可以把 minSdk 再往下压到 Compose 能撑住的档（约 26），
那时再改。
...
多读到一句用不着的话。正式包全部是 `READ`；`WRITE` 只在 `src/debug` Manifest 里，给开发页种子数据用。

**没有对等的 clinical / FHIR 读口。** iOS 的 `health_records` 走 HealthKit clinical records；
Android 这边不硬造一条假工具——化验单靠拍照 OCR（`RecognizedTextLayout`）和文档导入。
...
| App Intents / Siri | App Shortcuts / Assistant deep link（`vana://action/…`） | 前三条本地 `SpokenBrief` 不联网、不看 API key；「问 Vana」打开聊天自动发送 |
...
checkin/      早晚本地通知
exercises/    动作库与卡片
health/       Health Connect 读取、HealthSituation、HealthVitals、工具注册、DebugSeeder
intents/      SpokenBrief、App Shortcuts deep link
legal/        告知屏、隐私说明、急症规则
location/     粗定位 + 反地理编码
medications/  用药与补剂
memory/       长期记忆
...
- 健康数据正式包只读；API key 只进 `EncryptedSharedPreferences`。
