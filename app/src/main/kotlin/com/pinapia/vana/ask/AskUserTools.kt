...
object AskUserTools {
    ...
    fun registry(): CapabilityRegistry {
        val definition = CapabilityDefinition(
            name = ASK_TOOL_NAME,
            description = """
                反问用户一句，并把答案做成他点一下就能选的卡片。他的描述里**缺一个会改变你回答方向的条件、而它的取值只有有限几种**时就调用：是哪一种不舒服、什么时候开始的、想从哪儿入手，或者接下来能帮他的方向有好几个、该挑哪个得他说了算。**先问再答**，不要按最可能的那一种猜着答完。卡片显示在你这条回复下面，他也可以自己写一句或者跳过不答。测量卡片里已经记下的内容不要重复问。答案本身是开放的（他得讲一段经过）就直接用一句话问，别硬凑几个选项；一次只问一个问题。
            """.trimIndent().replace("\n", ""),
