...
object ConnectionTest {
    ...
    suspend fun run(providerId: String, modelId: String, rawApiKey: String): Result {
        ...
        if (!key.isValid) return Result.Failed(key.error ?: L10n.text("先填写 API 密钥。", "Enter an API key first."))
        val provider = CloudCatalog.provider(providerId)
            ?: return Result.Failed(L10n.text("先选择一个可用的 Provider。", "Choose an available provider first."))
        val wire = provider.wireProtocol
            ?: return Result.Failed(L10n.text("当前 Provider 的协议还不受支持。", "This provider's protocol is not supported."))
        if (modelId.isBlank()) return Result.Failed(L10n.text("先选择一个模型。", "Choose a model first."))
                ...
                client.stream(
                    AgentModelRequest(
                        ...
                        prompt = AgentTranscript(
                            listOf(AgentTranscript.Message.user("Reply with OK only")),
                        ),
